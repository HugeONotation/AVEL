...
namespace avel {
    ...
    vec8x64i broadcast_bit(mask8x64i m);
    ...
    [[nodiscard]]
    AVEL_FINL mask8x64i keep(mask8x64i m, mask8x64i v) {
        return mask8x64i{keep(mask8x64u{m}, mask8x64u{v})};
    }

    [[nodiscard]]
    AVEL_FINL mask8x64i clear(mask8x64i m, mask8x64i v) {
        return mask8x64i{clear(mask8x64u{m}, mask8x64u{v})};
    }

    [[nodiscard]]
    AVEL_FINL mask8x64i blend(mask8x64i m, mask8x64i a, mask8x64i b) {
        return mask8x64i{blend(mask8x64u{m}, mask8x64u{a}, mask8x64u{b})};
    }
    ...
    [[nodiscard]]
    AVEL_FINL vec8x64i broadcast_bit(mask8x64i m) {
        return vec8x64i{broadcast_bit(mask8x64u{m})};
    }
    ...
    AVEL_FINL vec8x64i average(vec8x64i a, vec8x64i b) {
        #if defined(AVEL_AVX512F)
        auto avg = (a & b) + ((a ^ b) >> 1);
        auto c = broadcast_bit((a < -b) | b == vec8x64i{std::int64_t(1) << 63}) & (a ^ b) & vec8x64i{1};
