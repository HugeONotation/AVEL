...
namespace avel_tests {
    ...
    TEST(Vec4x32i, No_bits_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits(vec4x32i{0})));
        EXPECT_FALSE(all(avel::no_bits(vec4x32i{-1})));
    }

    TEST(Vec4x32i, No_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4x32i>();
    ...
    TEST(Vec4x32i, No_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits_of(vec4x32i{0}, vec4x32i{0})));
        EXPECT_TRUE(all(avel::no_bits_of(vec4x32i{0}, vec4x32i{-1})));

        EXPECT_TRUE(all(avel::no_bits_of(vec4x32i{-1}, vec4x32i{0})));
        EXPECT_FALSE(all(avel::no_bits_of(vec4x32i{-1}, vec4x32i{-1})));
    }

    TEST(Vec4x32i, No_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr4x32i>();
            auto inputs1 = random_array<arr4x32i>();
    ...
    TEST(Vec4x32i, Any_bits_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits(vec4x32i{0})));
        EXPECT_TRUE(all(avel::any_bits(vec4x32i{-1})));
    }

    TEST(Vec4x32i, Any_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4x32i>();
    ...
    TEST(Vec4x32i, Any_bits_of_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits_of(vec4x32i{0}, vec4x32i{0})));
        EXPECT_FALSE(all(avel::any_bits_of(vec4x32i{0}, vec4x32i{-1})));

        EXPECT_FALSE(all(avel::any_bits_of(vec4x32i{-1}, vec4x32i{0})));
        EXPECT_TRUE(all(avel::any_bits_of(vec4x32i{-1}, vec4x32i{-1})));
    }

    TEST(Vec4x32i, Any_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr4x32i>();
            auto inputs1 = random_array<arr4x32i>();
    ...
    TEST(Vec4x32i, All_bits_edge_cases) {
        EXPECT_FALSE(all(avel::all_bits(vec4x32i{0})));
        EXPECT_TRUE(all(avel::all_bits(vec4x32i{-1})));
    }

    TEST(Vec4x32i, All_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4x32i>();
    ...
    TEST(Vec4x32i, All_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::all_bits_of(vec4x32i{0}, vec4x32i{0})));
        EXPECT_FALSE(all(avel::all_bits_of(vec4x32i{0}, vec4x32i{-1})));

        EXPECT_TRUE(all(avel::all_bits_of(vec4x32i{-1}, vec4x32i{0})));
        EXPECT_TRUE(all(avel::all_bits_of(vec4x32i{-1}, vec4x32i{-1})));
    }

    TEST(Vec4x32i, All_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr4x32i>();
            auto inputs1 = random_array<arr4x32i>();
