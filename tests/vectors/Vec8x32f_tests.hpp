...
namespace avel_tests {
    ...
    TEST(Mask8x32f, Keep_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8xb>();
            auto inputs1 = random_array<arr8xb>();

            mask8x32f v0{inputs0};
            mask8x32f v1{inputs1};
    ...
    */
    ...
    TEST(Vec8x32f, modf_edge_cases) {
        vec8x32f whole0;
        vec8x32f sig0 = avel::modf(vec8x32f{0.0f}, &whole0);
        EXPECT_TRUE(all(whole0 == vec8x32f{0.0f}));
        EXPECT_TRUE(all(sig0 == vec8x32f{0.0f}));

        vec8x32f whole1;
        vec8x32f frac1 = avel::modf(vec8x32f{-0.0f}, &whole1);
        EXPECT_TRUE(avel::compare_common_bytes(whole1, vec8x32f{-0.0f}));
        EXPECT_TRUE(avel::compare_common_bytes(frac1, vec8x32f{-0.0f}));

        vec8x32f whole2;
        vec8x32f frac2 = avel::modf(vec8x32f{NAN}, &whole2);
        EXPECT_TRUE(avel::all(avel::isnan(whole2)));
        EXPECT_TRUE(avel::all(avel::isnan(frac2)));

        vec8x32f whole3;
        vec8x32f frac3 = avel::modf(vec8x32f{+INFINITY}, &whole3);
        EXPECT_TRUE(avel::all(whole3 == vec8x32f{+INFINITY}));
        EXPECT_TRUE(avel::compare_common_bytes(frac3, vec8x32f{+0.0f}));

        vec8x32f whole4;
        vec8x32f frac4 = avel::modf(vec8x32f{-INFINITY}, &whole4);
        EXPECT_TRUE(avel::all(whole4 == vec8x32f{-INFINITY}));
        EXPECT_TRUE(avel::compare_common_bytes(frac4, vec8x32f{-0.0f}));
    }

    TEST(Vec8x32f, modf_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8x32f>();
            ...
            EXPECT_TRUE(all(results0 == vec8x32f{expected0}));
            EXPECT_TRUE(all(results1 == vec8x32f{expected1}));
        }
    }
