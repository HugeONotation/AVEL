...
namespace avel_tests {
    ...
    TEST(Vec64x8u, No_bits_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits(vec64x8u{0})));
        EXPECT_FALSE(all(avel::no_bits(vec64x8u(-1))));
    }

    TEST(Vec64x8u, No_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr64x8u>();
    ...
    TEST(Vec64x8u, No_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits_of(vec64x8u{0}, vec64x8u{0})));
        EXPECT_TRUE(all(avel::no_bits_of(vec64x8u{0}, vec64x8u(-1))));

        EXPECT_TRUE(all(avel::no_bits_of(vec64x8u(-1), vec64x8u{0})));
        EXPECT_FALSE(all(avel::no_bits_of(vec64x8u(-1), vec64x8u(-1))));
    }

    TEST(Vec64x8u, No_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr64x8u>();
            auto inputs1 = random_array<arr64x8u>();
    ...
    TEST(Vec64x8u, Any_bits_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits(vec64x8u{0})));
        EXPECT_TRUE(all(avel::any_bits(vec64x8u(-1))));
    }

    TEST(Vec64x8u, Any_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr64x8u>();
    ...
    TEST(Vec64x8u, Any_bits_of_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits_of(vec64x8u{0}, vec64x8u{0})));
        EXPECT_FALSE(all(avel::any_bits_of(vec64x8u{0}, vec64x8u(-1))));

        EXPECT_FALSE(all(avel::any_bits_of(vec64x8u(-1), vec64x8u{0})));
        EXPECT_TRUE(all(avel::any_bits_of(vec64x8u(-1), vec64x8u(-1))));
    }

    TEST(Vec64x8u, Any_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr64x8u>();
            auto inputs1 = random_array<arr64x8u>();
    ...
    TEST(Vec64x8u, All_bits_edge_cases) {
        EXPECT_FALSE(all(avel::all_bits(vec64x8u{0})));
        EXPECT_TRUE(all(avel::all_bits(vec64x8u(-1))));
    }

    TEST(Vec64x8u, All_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr64x8u>();
    ...
    TEST(Vec64x8u, All_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::all_bits_of(vec64x8u{0}, vec64x8u{0})));
        EXPECT_FALSE(all(avel::all_bits_of(vec64x8u{0}, vec64x8u(-1))));

        EXPECT_TRUE(all(avel::all_bits_of(vec64x8u(-1), vec64x8u{0})));
        EXPECT_TRUE(all(avel::all_bits_of(vec64x8u(-1), vec64x8u(-1))));
    }

    TEST(Vec64x8u, All_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr64x8u>();
            auto inputs1 = random_array<arr64x8u>();
