...
namespace avel_tests {
    ...
    TEST(Mask16x16i, Keep_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16xb>();
            auto inputs1 = random_array<arr16xb>();

            mask16x16i v0{inputs0};
            mask16x16i v1{inputs1};
    ...
    TEST(Vec16x16i, broadcast_bit_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16xb>();
    ...
    TEST(Vec16x16i, No_bits_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits(vec16x16i{0})));
        EXPECT_FALSE(all(avel::no_bits(vec16x16i{-1})));
    }

    TEST(Vec16x16i, No_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x16i>();
    ...
    TEST(Vec16x16i, No_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits_of(vec16x16i{0}, vec16x16i{0})));
        EXPECT_TRUE(all(avel::no_bits_of(vec16x16i{0}, vec16x16i{-1})));

        EXPECT_TRUE(all(avel::no_bits_of(vec16x16i{-1}, vec16x16i{0})));
        EXPECT_FALSE(all(avel::no_bits_of(vec16x16i{-1}, vec16x16i{-1})));
    }

    TEST(Vec16x16i, No_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16x16i>();
            auto inputs1 = random_array<arr16x16i>();
    ...
    TEST(Vec16x16i, Any_bits_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits(vec16x16i{0})));
        EXPECT_TRUE(all(avel::any_bits(vec16x16i{-1})));
    }

    TEST(Vec16x16i, Any_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x16i>();
    ...
    TEST(Vec16x16i, Any_bits_of_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits_of(vec16x16i{0}, vec16x16i{0})));
        EXPECT_FALSE(all(avel::any_bits_of(vec16x16i{0}, vec16x16i{-1})));

        EXPECT_FALSE(all(avel::any_bits_of(vec16x16i{-1}, vec16x16i{0})));
        EXPECT_TRUE(all(avel::any_bits_of(vec16x16i{-1}, vec16x16i{-1})));
    }

    TEST(Vec16x16i, Any_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16x16i>();
            auto inputs1 = random_array<arr16x16i>();
    ...
    TEST(Vec16x16i, All_bits_edge_cases) {
        EXPECT_FALSE(all(avel::all_bits(vec16x16i{0})));
        EXPECT_TRUE(all(avel::all_bits(vec16x16i{-1})));
    }

    TEST(Vec16x16i, All_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x16i>();
    ...
    TEST(Vec16x16i, All_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::all_bits_of(vec16x16i{0}, vec16x16i{0})));
        EXPECT_FALSE(all(avel::all_bits_of(vec16x16i{0}, vec16x16i{-1})));

        EXPECT_TRUE(all(avel::all_bits_of(vec16x16i{-1}, vec16x16i{0})));
        EXPECT_TRUE(all(avel::all_bits_of(vec16x16i{-1}, vec16x16i{-1})));
    }

    TEST(Vec16x16i, All_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16x16i>();
            auto inputs1 = random_array<arr16x16i>();
