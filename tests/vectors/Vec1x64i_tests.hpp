...
namespace avel_tests {
    ...
    TEST(Mask1x64i, Keep_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();

            mask1x64i v0{inputs0};
            mask1x64i v1{inputs1};
    ...
    TEST(Vec1x64i, broadcast_bit_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
    ...
    TEST(Vec1x64i, No_bits_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits(vec1x64i{0})));
        EXPECT_FALSE(all(avel::no_bits(vec1x64i{-1})));
    }

    TEST(Vec1x64i, No_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, No_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits_of(vec1x64i{0}, vec1x64i{0})));
        EXPECT_TRUE(all(avel::no_bits_of(vec1x64i{0}, vec1x64i{-1})));

        EXPECT_TRUE(all(avel::no_bits_of(vec1x64i{-1}, vec1x64i{0})));
        EXPECT_FALSE(all(avel::no_bits_of(vec1x64i{-1}, vec1x64i{-1})));
    }

    TEST(Vec1x64i, No_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Any_bits_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits(vec1x64i{0})));
        EXPECT_TRUE(all(avel::any_bits(vec1x64i{-1})));
    }

    TEST(Vec1x64i, Any_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Any_bits_of_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits_of(vec1x64i{0}, vec1x64i{0})));
        EXPECT_FALSE(all(avel::any_bits_of(vec1x64i{0}, vec1x64i{-1})));

        EXPECT_FALSE(all(avel::any_bits_of(vec1x64i{-1}, vec1x64i{0})));
        EXPECT_TRUE(all(avel::any_bits_of(vec1x64i{-1}, vec1x64i{-1})));
    }

    TEST(Vec1x64i, Any_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, All_bits_edge_cases) {
        EXPECT_FALSE(all(avel::all_bits(vec1x64i{0})));
        EXPECT_TRUE(all(avel::all_bits(vec1x64i{-1})));
    }

    TEST(Vec1x64i, All_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, All_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::all_bits_of(vec1x64i{0}, vec1x64i{0})));
        EXPECT_FALSE(all(avel::all_bits_of(vec1x64i{0}, vec1x64i{-1})));

        EXPECT_TRUE(all(avel::all_bits_of(vec1x64i{-1}, vec1x64i{0})));
        EXPECT_TRUE(all(avel::all_bits_of(vec1x64i{-1}, vec1x64i{-1})));
    }

    TEST(Vec1x64i, All_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
