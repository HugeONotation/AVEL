...
namespace avel_tests {
    ...
    TEST(Mask8x64i, Keep_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8xb>();
            auto inputs1 = random_array<arr8xb>();

            mask8x64i v0{inputs0};
            mask8x64i v1{inputs1};
    ...
    TEST(Mask8x64i, Blend_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8xb>();
            auto inputs1 = random_array<arr8xb>();
            auto inputs2 = random_array<arr8xb>();

            mask8x64i v0{inputs0};
            mask8x64i v1{inputs1};
            mask8x64i v2{inputs2};
    ...
    TEST(Vec8x64i, broadcast_bit_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8xb>();
    ...
    TEST(Vec8x64i, No_bits_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits(vec8x64i{0})));
        EXPECT_FALSE(all(avel::no_bits(vec8x64i{-1})));
    }

    TEST(Vec8x64i, No_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8x64i>();
    ...
    TEST(Vec8x64i, No_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::no_bits_of(vec8x64i{0}, vec8x64i{0})));
        EXPECT_TRUE(all(avel::no_bits_of(vec8x64i{0}, vec8x64i{-1})));

        EXPECT_TRUE(all(avel::no_bits_of(vec8x64i{-1}, vec8x64i{0})));
        EXPECT_FALSE(all(avel::no_bits_of(vec8x64i{-1}, vec8x64i{-1})));
    }

    TEST(Vec8x64i, No_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8x64i>();
            auto inputs1 = random_array<arr8x64i>();
    ...
    TEST(Vec8x64i, Any_bits_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits(vec8x64i{0})));
        EXPECT_TRUE(all(avel::any_bits(vec8x64i{-1})));
    }

    TEST(Vec8x64i, Any_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8x64i>();
    ...
    TEST(Vec8x64i, Any_bits_of_edge_cases) {
        EXPECT_FALSE(all(avel::any_bits_of(vec8x64i{0}, vec8x64i{0})));
        EXPECT_FALSE(all(avel::any_bits_of(vec8x64i{0}, vec8x64i{-1})));

        EXPECT_FALSE(all(avel::any_bits_of(vec8x64i{-1}, vec8x64i{0})));
        EXPECT_TRUE(all(avel::any_bits_of(vec8x64i{-1}, vec8x64i{-1})));
    }

    TEST(Vec8x64i, Any_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8x64i>();
            auto inputs1 = random_array<arr8x64i>();
    ...
    TEST(Vec8x64i, All_bits_edge_cases) {
        EXPECT_FALSE(all(avel::all_bits(vec8x64i{0})));
        EXPECT_TRUE(all(avel::all_bits(vec8x64i{-1})));
    }

    TEST(Vec8x64i, All_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8x64i>();
    ...
    TEST(Vec8x64i, All_bits_of_edge_cases) {
        EXPECT_TRUE(all(avel::all_bits_of(vec8x64i{0}, vec8x64i{0})));
        EXPECT_FALSE(all(avel::all_bits_of(vec8x64i{0}, vec8x64i{-1})));

        EXPECT_TRUE(all(avel::all_bits_of(vec8x64i{-1}, vec8x64i{0})));
        EXPECT_TRUE(all(avel::all_bits_of(vec8x64i{-1}, vec8x64i{-1})));
    }

    TEST(Vec8x64i, All_bits_of_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8x64i>();
            auto inputs1 = random_array<arr8x64i>();
