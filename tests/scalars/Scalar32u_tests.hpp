...
namespace avel_tests {
    ...
    */
    ...
    TEST(Scalar32u, No_bits_preselected) {
        EXPECT_TRUE(avel::no_bits(std::uint32_t(0x00000000)));

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT; ++i) {
            std::uint32_t x = std::uint32_t(1) << i;
            EXPECT_FALSE(avel::no_bits(x));
        }

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT - 1; ++i) {
            std::uint32_t x = std::uint32_t(0xffffffff) >> (sizeof(std::uint32_t) * CHAR_BIT - 1 - i);
            EXPECT_FALSE(avel::no_bits(x));
        }
    }

    TEST(Scalar32u, No_bits_random) {
    ...
    TEST(Scalar32u, No_bits_of_preselected) {
        EXPECT_TRUE(avel::no_bits_of(std::uint32_t(0x00000000), std::uint32_t(0x00000000)));

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT; ++i) {
            std::uint32_t x = std::uint32_t(1) << i;
            EXPECT_TRUE(avel::no_bits_of(x, std::uint32_t(0x00000000)));
        }

        EXPECT_TRUE(avel::no_bits_of(std::uint32_t(0x00000000), std::uint32_t(0xffffffff)));

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT - 1; ++i) {
            std::uint32_t x = std::uint32_t(0xffffffff) >> (sizeof(std::uint32_t) * CHAR_BIT - 1 - i);
            EXPECT_FALSE(avel::no_bits_of(x, std::uint32_t(0xffffffff)));
        }
    }

    TEST(Scalar32u, No_bits_of_random) {
        for (std::int32_t i = 0; i < iterations; ++i) {
            std::uint32_t x = random32u();
            std::uint32_t m = random32u();

            bool expected = true;
            for (std::int32_t j = 0; j < sizeof(std::uint32_t) * CHAR_BIT; ++j) {
                bool x_bit = (x >> j) & 0x1;
                bool m_bit = (m >> j) & 0x1;
    ...
    TEST(Scalar32u, Any_bits_preselected) {
        EXPECT_FALSE(avel::any_bits(std::uint32_t(0x00000000)));
    ...
    TEST(Scalar32u, Any_bits_random) {
        for (std::int32_t i = 0; i < iterations; ++i) {
            std::uint32_t x = random32u();
    ...
    TEST(Scalar32u, Any_bits_of_preselected) {
        EXPECT_FALSE(avel::any_bits_of(std::uint32_t(0x00000000), std::uint32_t(0x00000000)));

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT; ++i) {
            std::uint32_t x = std::uint32_t(1) << i;
            EXPECT_FALSE(avel::any_bits_of(x, std::uint32_t(0x00000000)));
        }

        EXPECT_FALSE(avel::any_bits_of(std::uint32_t(0x00000000), std::uint32_t(0xffffffff)));

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT - 1; ++i) {
            std::uint32_t x = std::uint32_t(0xffffffff) >> (sizeof(std::uint32_t) * CHAR_BIT - 1 - i);
            EXPECT_TRUE(avel::any_bits_of(x, std::uint32_t(0xffffffff)));
        }
    }

    TEST(Scalar32u, Any_bits_of_random) {
        for (std::int32_t i = 0; i < iterations; ++i) {
            std::uint32_t x = random32u();
            std::uint32_t m = random32u();
    ...
    TEST(Scalar32u, All_bits_preselected) {
        EXPECT_FALSE(avel::all_bits(std::uint32_t(0x00000000)));
        ...
        EXPECT_TRUE(avel::all_bits(std::uint32_t(0xffffffff)));
    }

    TEST(Scalar32u, All_bits_random) {
        for (std::int32_t i = 0; i < iterations; ++i) {
            std::uint32_t x = random32u();
    ...
    TEST(Scalar32u, All_bits_of_preselected) {
        EXPECT_TRUE(avel::all_bits_of(std::uint32_t(0x00000000), std::uint32_t(0x00000000)));

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT; ++i) {
            std::uint32_t x = std::uint32_t(1) << i;
            EXPECT_TRUE(avel::all_bits_of(x, std::uint32_t(0x00000000)));
        }

        for (std::uint32_t i = 0; i < sizeof(std::uint32_t) * CHAR_BIT - 1; ++i) {
            std::uint32_t x = std::uint32_t(0xffffffff) >> (sizeof(std::uint32_t) * CHAR_BIT - 1 - i);
            EXPECT_FALSE(avel::all_bits_of(x, std::uint32_t(0xffffffff)));
        }

        EXPECT_TRUE(avel::all_bits_of(std::uint32_t(0xffffffff), std::uint32_t(0xffffffff)));
    }

    TEST(Scalar32u, All_bits_of_random) {
        for (std::int32_t i = 0; i < iterations; ++i) {
            std::uint32_t x = random32u();
            std::uint32_t m = random32u();
